...
The implementation of a standard library of components only involves extending the base Component type and adding the necessary phases alongside some component specific functionality. 

The communication between components is facilitated by channels. However, in the context of components, it is benificial to think about component ports and connecting them. Like this, it is not necessary to directly pass a reference to a channel object to the components which should be connected through a channel. Instead, both declare ports, one a \ttt{ReceiverPort} and one a \ttt{SenderPort}. The parent component can then connect the two ports by calling the \ttt{connect} method. 

\todo{briefly describe impl: ports, helper methods, phases}
