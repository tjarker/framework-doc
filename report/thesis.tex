...
clock domain that they are associated with. The option of adding skews like in Vera, could be added in the future. In
order to support multiple clocks, the explicit definition of periods has to be possible.

This information will be captured in an interface definition which captures the inputs and outputs of the DUT in
...
\todo{talk about implicit clock -> sometimes its annoying to always pass clock to be able to manipulate time -> a
  context should provide a default clock \ttt{stepClockDomain(n)}, and the use can create new contexts using
\ttt{withClockDomain(clk)}}
