...
\begin{listing}
\begin{lstlisting}[language=scala, captionpos=b, caption=???.,label=lst:factory]
Comp.create[MyComponent](
  "param1" -> 10,
  Param.Msg -> "hello"
) // additional parameters can be added to specific components

val paramMap: Map[Any, Any] 
Comp.create[MyComponent](paramMap)

Comp.create[MyComponent]

Comp.overrideType[MyComponent, MyDerivedComponent]

Comp.set(Param.Msg, "hello")
Comp.get(Param.Msg)

Comp.set(child, Param.Msg, "updated message")


Comp.builder[MyComponent]
  .with("param1" -> 10)
  .with(Param.Msg -> "hello")
  .withOverride[MyComponent, MyDerivedComponent]
  .build()
\end{lstlisting}
\end{listing}
